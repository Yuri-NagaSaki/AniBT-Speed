...
| 单次删除上限 | 20 | 每次清理最多删除的种子数，防止误删过多种子 |
...
系统每 3 分钟检查一次所有种子的 Peer 状态。默认采用“热窗口 + 轮转探测”：新种先保持持续做种，旧种无下载者时暂停；暂停旧种按批次短暂恢复探测，窗口内有人下载就保留做种，否则再次暂停。
...
| 热窗口 | 12 小时 | 添加后不满此时间的新种持续做种 |
| 排除标签 | 空 | 带有这些标签的种子不参与队列管理 |
| 探测暂停旧种 | 开启 | 定时短暂恢复一批暂停旧种以捕捉短暂下载者 |
| 探测间隔 | 5 分钟 | 每隔多久启动一批旧种探测 |
| 探测批量 | 50 个 | 每轮最多短暂恢复多少个暂停种子 |
| 探测窗口 | 3 分钟 | 探测种子保持运行的时间 |
| 月龄清理 | 关闭 | 删除完成做种超过指定月数的种子和文件 |
| 月龄阈值 | 3 月 | 按 30 天/月计算 |

暂停条件：

超过热窗口后，正在做种（uploading、stalledUP 或 forcedUP 状态）的种子，当其在所有 Tracker 及 DHT 网络上的下载者（leechers）数为 0 时，系统判定该种子暂时无人需要下载，自动暂停以节约队列、CPU 和内存资源。

恢复条件：

已暂停的种子（pausedUP 或 qB 5.x 的 stoppedUP），当检测到有新的下载者出现（leechers 数量大于恢复阈值）时，系统自动恢复做种。为避免小体积种子的下载者在轮询间隔内消失，系统还会按“探测间隔”轮转恢复一批暂停旧种；探测窗口结束后仍无下载者且无上传速度，则再次暂停。

清理条件：

关键词清理按种子名称匹配；月龄清理按完成做种时间计算，默认关闭，开启后可设置 X 个月作为阈值。两类清理都会遵守排除标签和每轮上限，避免一次误删过多。
...
| 速率检查 | 1 分钟 | 根据状态动态调整限速 |
| 流量记录 | 1 分钟 | 记录各实例流量数据用于趋势图和统计 |
...
系统使用 JWT 认证，通过 `.env` 中的 `ADMIN_PASSWORD` 设置登录密码。Token 有效期 24 小时，退出登录后 Token 立即失效。
...
## MediaInfo Agent（独立部署，用户不要部署使用，这个是我做种拿mediainfo使用的，你们无法推送的）
...
- **info_hash 匹配**：通过种子 info_hash 自动匹配 Release，无需手动指定 releaseId
