...
系统每 3 分钟检查一次所有种子的 Peer 状态。默认保持完成种子持续做种，确保小体积种子也能立即响应短暂下载请求；如需节省资源，可开启“无下载者暂停”。
...
开启“无下载者暂停”后，正在做种（uploading、stalledUP 或 forcedUP 状态）的种子，当其在所有 Tracker 及 DHT 网络上的下载者（leechers）数为 0 时，系统判定该种子已广泛传播，无人需要下载，自动暂停以节约带宽、CPU 和内存资源。

恢复条件：

已暂停的种子（pausedUP 或 qB 5.x 的 stoppedUP），当检测到有新的下载者出现（leechers 数量大于恢复阈值）时，系统自动恢复做种。对于体积很小、下载窗口很短的种子，建议保持“无下载者暂停”关闭。
