  ...
  return (
    <SettingsForm
      ...
      fields={[
        ...
        { key: 'probe_interval_minutes', label: '探测间隔', type: 'number', unit: '分钟', help: '每隔多久启动一批旧种探测；越短越不容易错过短暂下载者', min: 3, max: 240 },
        { key: 'probe_batch_size', label: '探测批量', type: 'number', unit: '个', help: '每轮最多短暂恢复多少个暂停种子；调大可提高覆盖率', min: 1, max: 1000 },
        ...
        { key: 'age_cleanup_enabled', label: '启用月龄清理', type: 'toggle', help: '开启后会删除完成做种超过指定月数的种子和文件' },
        { key: 'age_cleanup_months', label: '月龄清理阈值', type: 'number', unit: '月', help: '按 30 天/月计算；例如 3 表示完成做种超过约 90 天后清理', min: 1, max: 36 },
        { key: 'age_cleanup_max_per_run', label: '月龄每轮最多清理', type: 'number', unit: '个', help: '每次队列巡检最多删除多少个月龄命中的种子，防止一次清理过多', min: 1, max: 200 },
      ]}
    />
  )
}
